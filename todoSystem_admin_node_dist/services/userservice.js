...
function getUserByAccountAndPassword({user_account, user_password}, callback) {
    ...
    pool.query(
        "select user_id from todo_user where user_account=? and user_password=?",
...
function getUserInfoByToken({token}, callback) {
    verify(token, (err, {user_id}) => {
        ...
        pool.query(
            `select
                        username,user_account,user_mobile,user_address,user_sex
                    from todo_user where user_id=?`,
            [user_id],
            callback
        )
    })
